...
module.exports.run = async (client, message, args) => {
    ...
    let mbr = await client.users.cache.filter(member => !member.bot && member.presence.status == "dnd" || member.presence.status == "idle" || member.presence.status == "online").size;
    let scd = mbr*0.12;
    scd= scd * 1000;
    ...
    let memberarray = client.users.cache.filter(member => member.presence.status == "dnd" || member.presence.status == "idle" || member.presence.status == "online").array();
